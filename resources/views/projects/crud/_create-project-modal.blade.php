<div class="individual-project-create-modal-bg"></div>

<div class="animated fadeIn individual-project-create-modal-container">

  @if(Request::is('projects/*'))
    <h1 class="individual-project-create-modal-title">Create New Project</h1>
  @elseif(Request::is('tasks/*'))
    <h1 class="individual-project-create-modal-title">Create New Task</h1>
  @endif

  <a href="#!"><img class="individual-project-create-modal-close-btn" src="data:image/svg+xml;utf8;base64," /></a>

  <div class="clear"></div>

  <form class="" action="{{ route('projects.store') }}" method="POST">

    {{ method_field('POST') }}

    {{ csrf_field() }}

    <h2 class="individual-project-create-modal-sub-title">Title</h2>

    <input class="individual-project-create-modal-input" placeholder="Title" type="text" name="name"><br>

    <h2 class="individual-project-create-modal-sub-title">Description</h2>

    <textarea class="individual-project-create-modal-textarea" placeholder="Description" name="description"></textarea><br>

    <h2 class="individual-project-create-modal-sub-title">Deadline</h2>

    <input class="individual-project-create-modal-deadline" placeholder="Deadline" type="text" id="datepicker" name="deadline">

    <div class="clear"></div><br>

    <input class="individual-project-create-modal-submit" type="submit" name="submit" value="Create">

  </form>

</div>
